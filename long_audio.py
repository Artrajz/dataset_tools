...
def clip(audio_path, rec_result, input_base_path, output_dir="datasets", min_time=3000):
...
def clip_all(input_path, output_dir="datasets"):
    audio_files = glob.glob(os.path.join(input_path, "**", "*.*"), recursive=True)
    lines = []
    for audio_path in audio_files:
        rec_result = recog(audio_path)

        lines.extend(clip(audio_path, rec_result, input_base_path=input_path, output_dir=output_dir, min_time=3000))
